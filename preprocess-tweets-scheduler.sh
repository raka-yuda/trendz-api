...
script_path="/src/runners/preprocess-tweets.js"
...
log_file="$log_folder_path/log-preprocess-tweets-$timestamp.log"

# Change directory to script directory
cd $script_directory

# Use nohup to run the Node.js script in the background and redirect output to the log file
nohup node --experimental-modules "$script_directory/$script_path" >> "$log_file" 2>&1 &